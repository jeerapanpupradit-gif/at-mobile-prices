...
document.addEventListener("DOMContentLoaded", () => {
  ...
  phoneForm.addEventListener("submit", async (e) => {
    ...
    try {
      if (targetId) {
        ...
        const repairsList = [
          { type: "screen", name: brand === "Apple" ? "เปลี่ยนหน้าจอแท้ (Original Screen)" : (brand === "Samsung" ? "เปลี่ยนหน้าจอแท้ศูนย์ (Original Screen)" : "เปลี่ยนหน้าจอแท้ (Original Screen)"), price: 0, time: "2 ชม.", warranty: "90 วัน" },
          { type: "battery", name: brand === "Apple" ? "เปลี่ยนแบตเตอรี่แท้ (Original Battery)" : (brand === "Samsung" ? "เปลี่ยนแบตเตอรี่ศูนย์ (Original Battery)" : "เปลี่ยนแบตเตอรี่แท้ (Original Battery)"), price: 0, time: "1 ชม.", warranty: "180 วัน" },
          { type: "backglass", name: brand === "Apple" ? "เปลี่ยนกระจกฝาหลัง (Back Glass)" : (brand === "Samsung" ? "เปลี่ยนฝาหลังแท้ (Back Glass)" : "เปลี่ยนฝาหลังแท้ (Back Glass)"), price: 0, time: "2 ชม.", warranty: "90 วัน" },
          { type: "motherboard", name: brand === "Apple" ? "ซ่อมเมนบอร์ด (Motherboard)" : "ซ่อมบอร์ด (Motherboard)", price: 0, time: "1-2 วัน", warranty: "90 วัน" },
          { type: "camera", name: "เปลี่ยนกล้องหลัง (Rear Camera)", price: 0, time: "2 ชม.", warranty: "90 วัน" },
          { type: "charging", name: brand === "Apple" ? "เปลี่ยนแพตูดชาร์จ (Charging Port)" : "เปลี่ยนตูดชาร์จ (Charging Port)", price: 0, time: "1 ชม.", warranty: "90 วัน" }
        ];

        const newPhone = await Database.addPhone({
          brand,
          model,
          color,
          sellingPrices: capacityItems,
        ...
        if (newPhone && newPhone.id) {
          selectedRepairPhoneId = newPhone.id;
        }
      }
      ...
      alert(targetId ? "แก้ไขข้อมูลโทรศัพท์เรียบร้อยแล้ว" : "เพิ่มโทรศัพท์รุ่นใหม่เรียบร้อยแล้ว");
    } catch (err) {
  ...
  // Helper to check empty placeholder
  function checkEmptyRepairsState() {
    const listContainer = document.getElementById("repair-items-form-list");
    const placeholder = document.getElementById("repair-empty-placeholder");
  ...
  // Create a dynamic repair item card element
  function createRepairItemRow(rep = { type: "", name: "", price: 0, time: "1 ชม.", warranty: "90 วัน" }) {
    const itemDiv = document.createElement("div");
    itemDiv.className = "repair-item-form";
    itemDiv.setAttribute("data-type", rep.type || `custom-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`);
    
    itemDiv.innerHTML = `
      <div class="repair-header-form">
        <input type="text" class="form-input rep-name" style="font-weight: 600; font-size: 0.85rem; border: none; border-bottom: 1px dashed var(--border-color); border-radius: 0; padding: 4px 0; background: transparent; width: 70%;" required placeholder="ระบุบริการซ่อม (เช่น เปลี่ยนกล้องหน้า)" value="${rep.name}">
        <button type="button" class="remove-repair-item-btn"><i class="fa-solid fa-trash"></i> ลบ</button>
      </div>
    ...
    itemDiv.querySelector(".remove-repair-item-btn").addEventListener("click", () => {
      itemDiv.remove();
      checkEmptyRepairsState();
    });
  ...
  function openRepairModal(phoneToEdit) {
    ...
    repairModalBody.innerHTML = `
      <div id="repair-empty-placeholder" style="text-align: center; padding: 20px; color: var(--text-secondary); display: none;">
        ไม่มีรายการบริการซ่อมสำหรับโทรศัพท์รุ่นนี้ กดปุ่มด้านล่างเพื่อเพิ่ม
      </div>
      <div id="repair-items-form-list"></div>
      <div style="margin-top: 16px;">
        <button type="button" class="btn-secondary" id="btn-add-repair-item" style="width: 100%; border-style: dashed; justify-content: center; border-radius: 20px;">
          <i class="fa-solid fa-plus"></i> เพิ่มรายการบริการซ่อม
        </button>
      </div>
    `;
    
    const listContainer = document.getElementById("repair-items-form-list");
    const btnAddRepairItem = document.getElementById("btn-add-repair-item");
    
    if (phoneToEdit.repairs && phoneToEdit.repairs.length > 0) {
      phoneToEdit.repairs.forEach((rep) => {
        const itemRow = createRepairItemRow(rep);
    ...
    btnAddRepairItem.addEventListener("click", () => {
      const newItemRow = createRepairItemRow({
        type: `custom-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
        name: "",
        price: 0,
        time: "1 ชม.",
        warranty: "90 วัน"
      });
      listContainer.appendChild(newItemRow);
      checkEmptyRepairsState();
      
      // Auto-scroll the modal body to show the newly added item
      newItemRow.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      // Focus on the name input of the new item
      newItemRow.querySelector(".rep-name").focus();
    });
  ...
  repairForm.addEventListener("submit", async (e) => {
    ...
    const repairGroups = listContainer.querySelectorAll(".repair-item-form");
    
    repairGroups.forEach(group => {
      const type = group.getAttribute("data-type");
      const name = group.querySelector(".rep-name").value.trim();
    ...
    try {
      ...
      alert("บันทึกราคาและรายการซ่อมเรียบร้อยแล้ว");
    } catch (err) {
      console.error("Save repairs error", err);
      alert("เกิดข้อผิดพลาดในการบันทึกข้อมูลราคาซ่อม: " + err.message);
    } finally {
      submitBtn.disabled = false;
      submitBtn.innerHTML = originalBtnText;
    }
